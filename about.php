<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <title>Naktya Chhen - About Us</title>
        <meta name="description" content="">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <?php require('include/head-links.php')?>
    </head>
    <body>
        
        <?php require('include/header.php')?>
        <main class="ph-about-page ph-innerpage">
            <section class="ph-banner">
                <div class="">
                    <div class="ph-banner__wrapper">
                        <div class="ph-banner__item">
                            <div class="ph-banner__item--img">
                                <img src="assets/img/banner.jpg" alt="">
                            </div>
                            <div class="ph-banner__item--content">
                                <div class="ph-banner__item--details">
                                    
                                <h2>
                                    Our Culinary Maestros
                                </h2>
                                <p>
                                Behind every  dish, there's a passion-driven story
                                </p>
                                </div>
                                <div class="ph-breadcrums"><div class="container">
                                    <ul class="ph-breadcrums__list justify-content-center">
                                        <li class="item">
                                            <a href="index.php" title="Go to Home Page">
                                                Home </a>
                                        </li>
                                        <li class="item mt_page">
                                            <strong>About Us</strong>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <section class="ph-frame ph-section__padding-lg--tb text-center">
                <div class="container">
                    <div class="row justify-content-center">
                        
                        <div class="col-12 col-md-10 mx-auto text-center ph-section__padding-sm--b">
                            <div class="ph-section__title wow  fadeInUpBig">
                                <h2>
                                A Symphony of real Talent
                                </h2>
                                <p>
                                Get to know the masterminds shaping your dining experience
                                </p>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12 col-md-6 col-lg-3">
                            <div class="ph-frame__item wow  fadeInUpBig">
                                <div class="ph-frame__item--img">
                                    <figure>
                                        <img src="assets/img/about-chef-1.png" alt="">
                                    </figure>
                                </div>
                                <div class="ph-frame__item--content ">
                                    <h4>
                                    Greetings! I'm Kalyan Katwal, your Manager
                                    </h4>
                                    <p>
                                        I ensure a seamless dining experience, overseeing operations to make your visit memorable
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-lg-3">
                            <div class="ph-frame__item wow  fadeInUpBig" data-wow-delay="0.1s">
                                <div class="ph-frame__item--img">
                                    <figure>
                                        <img src="assets/img/about-chef-2.png" alt="">
                                    </figure>
                                </div>
                                <div class="ph-frame__item--content">
                                    <h4>
                                    Welcome! I'm Chef Aditya Acharya
                                    </h4>
                                    <p>
                                        I leading the kitchen with passion and creativity, preaparing delectable dishes to delight your palate
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-lg-3">
                            <div class="ph-frame__item wow  fadeInUpBig" data-wow-delay="0.2s">
                                <div class="ph-frame__item--img">
                                    <figure>
                                        <img src="assets/img/about-chef-3.png" alt="">
                                    </figure>
                                </div>
                                <div class="ph-frame__item--content">
                                    <h4>
                                    Hello! I'm Jatan Adhikari, your Front House Manager
                                    </h4>
                                    <p>
                                        I ensure exceptional service and ambiance, making your dining experience enjoyable and memorable
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-lg-3">
                            <div class="ph-frame__item  wow  fadeInUpBig" data-wow-delay="0.3s">
                                <div class="ph-frame__item--img">
                                    <figure>
                                        <img src="assets/img/about-chef-4.png" alt="">
                                    </figure>
                                </div>
                                <div class="ph-frame__item--content">
                                    <h4>
                                    Cheers! I'm Salina, your Bartender
                                    </h4>
                                    <p>
                                        Crafting exquisite cocktails and providing friendly service to enhance your time at our bar
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <section class="ph-component">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12 col-md-6 g-0">
                            <div class="ph-component__img">
                                <figure>
                                    <img src="assets/img/about-feature.jpg" alt="">
                                </figure>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 g-0">
                            <div class="ph-component__content wow fadeInRightBig">
                                <div class="ph-section__title">
                                    <h2>
                                    We Are Experienced Restaurant.
                                    </h2>
                                    <h3>
                                        Welcome our restaurant! Our Restaurant is the best as like delicious food, nutrition food etc in world-wide.
                                    </h3>
                                </div>
                                <div class="ph-section__content">
                                    <p>
                                        It was popularised in the 2000s.
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <section class="ph-about">
                <div class="ph-center ph-section__padding-lg--tb">
                    <div class="ph-about__wrapper">
                        <div class="container">
                            <div class="row text-center">
                                <div class="col-12 col-md-10 mx-auto text-center">
                                    <div class="ph-section__title ph-section__padding-sm--b">
                                        <h2 class="wow  fadeInUpBig">
                                        Embark on a fooding Journey with Us
                                        </h2>
                                        <h3 class="wow  fadeInUpBig">
                                        Discover Unmatched Hospitality, Fooding Excellence, and Captivating Ambiance
                                        </h3>
                                    </div>
                                </div>
                                <div class="col-12 col-md-6 col-xl-4">
                                    <div class="ph-about__content wow  fadeInUpBig">
                                        <figure>
                                            <img src="assets/img/about-content-1.jpg" alt="">
                                        </figure>
                                        <p>
                                        Our restaurant is not just a place to eat; it's a vibrant hub where friends gather for quick bites and meaningful conversations. Whether it's a casual catch-up or a cozy hangout, our welcoming atmosphere is perfect for every occasion.
                                        </p>
                                    </div>
                                </div>
                                <div class="col-12 col-md-6 col-xl-4">
                                    <div class="ph-about__content wow  fadeInUpBig">
                                        <figure>
                                            <img src="assets/img/about-content-2.jpg" alt="">
                                        </figure>
                                        <p>
                                        Explore a diverse range of food options that promise to tantalize your senses. From delectable appetizers to mouthwatering main courses and irresistible desserts, each dish is a masterpiece that reflects our commitment to culinary excellence.
                                        </p>
                                    </div>
                                </div>
                                <div class="col-12 col-lg-4">
                                    <div class="ph-about__content wow  fadeInUpBig">
                                        <figure>
                                            <img src="assets/img/about-content-3.jpg" alt="">
                                        </figure>
                                        <p>
                                        Experience the magic of our restaurant at night, where the ambiance comes alive with a mesmerizing charm that complements your dining experience, making every visit truly unforgettable.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <?php require('include/newsletter.php')?>
        </main>
        <?php require('include/footer.php')?>
        <?php require('include/script.php')?>
        
    </body>
</html>